...
def cart(request):
    ...
    if cart_obj:
        ...
        total_amount = int(cart_obj.get_cart_total_price_after_coupon() * 100)
        
        if total_amount < 100:
            messages.warning(request, 'Total amount must be at least $1.00')
            return redirect('index')

        intent = stripe.PaymentIntent.create(
            amount=total_amount,
            currency='usd',
            metadata={'cart_id': str(cart_obj.uid)}
        )

    context = {
        'cart': cart_obj,
        'client_secret': intent.client_secret if cart_obj else None,
